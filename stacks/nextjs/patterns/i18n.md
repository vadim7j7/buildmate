...
> **The locale set is fixed at bootstrap and identical to the mobile app.** It is already
> scaffolded into `src/i18n/config.ts`, `src/i18n/routing.ts`, `src/i18n/request.ts`,
> `src/middleware.ts`, and `next.config.ts`. **Do not hardcode a locale array** — derive
> everything from `locales`/`defaultLocale` in `src/i18n/config.ts`. To change which
> languages are supported, change the `languages` option, not these files.
...
`src/i18n/config.ts` is already generated with the project's configured locale set. It
exports `locales`, `defaultLocale`, and `localeNames`. Reference those everywhere — never
re-list locales inline:

```typescript
// src/i18n/config.ts (generated — locales come from the configured set)
...
export const config = {
  // Don't enumerate locales in the matcher — exclude API/_next/static instead,
  // so it works for any configured locale set.
