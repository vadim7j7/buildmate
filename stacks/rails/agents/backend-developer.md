...
description: Rails backend developer specializing in services, controllers, models, and jobs
...
1. **Auto-fix lint**: `bundle exec rubocop -A <changed_files>`
2. **Verify lint**: `bundle exec rubocop <changed_files>` (must be zero offenses)
3. **Run specs**: `bundle exec rspec <related_spec_files>` (must all pass)
