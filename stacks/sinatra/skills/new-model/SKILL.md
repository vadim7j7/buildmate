---
name: new-model
description: Generate an ActiveRecord model for Sinatra with migration and spec
---
...
Generates an ActiveRecord model for use with Sinatra, including migration file,
model class, and RSpec test.
...
4. **Generate the model file.** Create the model with:
   ...
   - Inheriting from `ActiveRecord::Base` (or `ApplicationRecord`)
