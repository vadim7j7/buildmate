...
2. **Third-party** -- UI library imports, form libraries, utilities
...
// 2. Third-party (UI library, utilities, etc.)
import { z } from 'zod';
...
| UI library layout components only (cards, stacks, text) | No (usually) |
  ...
  return (
    <div className={`card ${isSelected ? 'card-selected' : ''}`}>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => onDelete(project.id)} className="btn-danger">Delete</button>
      </div>
    </div>
  );
}
...
export function ProjectCard(props: ProjectCardProps) {
  return <h3>{props.project.name}</h3>;
}
```
...
For form submissions, show user feedback via your UI library's notification system:
...
## 13. ESLint and Prettier Enforcement
